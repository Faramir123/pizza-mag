...
function PizzaBlock({name, imageUrl, price, sizes, types}) {
  ...
  const arrTypePrice = [0, 270]
  const typePrice = 0

  const handleSelectSize = (index) =>{
    setChooseSize(index)
    console.log(arrSizePrice[index])
    setAmout(price + arrSizePrice[index])

  }
  const handleSelectTypes = (index) =>{
    setChooseTypes(index)
    typePrice = arrTypePrice[index]
    return typePrice
  }

    const amountPrice = typePrice + amout
    ...
    return (
        ...
        <div className="pizza-block__bottom">
          <div className="pizza-block__price">{amountPrice} ₽</div>

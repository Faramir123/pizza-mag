...
function PizzaBlock({name, imageUrl, price, sizes, types}) {
  ...
  const [calcPrice, setCulcPrice] = React.useState(price)
  const avaibleTypes = ['тонкое', 'традиционное']
  const avaibleSizes = [26,30,40]
  const typePrice = [0, 100]
  const sizePrice = [0, 120, 270]

  const handleSelectSize = (index) =>{
    setChooseSize(index)
    setCulcPrice(price = calcPrice + sizePrice[index])
  }
  const handleSelectTypes = (index) =>{
    setChooseTypes(index)
    setCulcPrice(price = calcPrice + typePrice[index])
  }
    ...
    return (
        ...
        <div className="pizza-block__bottom">
          <div className="pizza-block__price">от { calsPrice } ₽</div>
